...
| S3 | 資訊列・禮金 | 同上 | 卡內第7.3列・滿寬 | 標籤 `禮金` ／ 值 `女方親友席不收禮金，敬請入座，您的到來就是我們最珍貴的祝福` | icon `heart`；**僅蔬食版出現**，見下方說明 |
| S3 | 資訊列・餐點安排 | 同上 | 卡內第7.5列・滿寬（**在桌次引導之前**） | 標籤 `餐點安排` ／ 值 `女方親友席宴請蔬食料理，由飯店主廚特別設計，期待與您共享蔬食盛宴` | icon `leaf`；**僅蔬食版出現**，見下方說明 |
...
同一份畫面有兩個網址：`/`（葷食，預設）與 `/vegetarian`（蔬食）。差異全部集中在程式的
`DIET_VARIANTS`，**設計檔只畫葷食版**。

⚠️ **兩個網址實際的分流不是「吃葷／吃素」，而是「女方親戚桌／其他所有人」。**
女方親戚桌整桌全蔬食，拿 `/vegetarian`；女方朋友（桌次葷素混合）與男方賓客都拿 `/`。
這是「禮金」與「餐點安排」兩列能寫得這麼直接的前提 —— 讀者範圍與那兩句話的對象完全吻合。
⛔ 日後若把蔬食連結也發給其他吃素的賓客，這兩句都要重寫，蔬食版的欄位預告也要把
「素食需求」加回來（現在拿掉是因為讀者確定都吃蔬食席）。

| 項目 | 葷食版 | 蔬食版 |
| --- | --- | --- |
| S3 桌次引導的值 | 入口處設有座位表，男方親友請至 A 區、女方親友請至 B 區 | 入口處設有座位表，我們已為您預留蔬食席位 |
| S3 禮金這一列 | 不出現 | 出現，**排在餐點安排之前** |
| S3 餐點安排這一列 | 不出現 | 出現，**排在桌次引導之前** |
| S4 欄位預告 chips | 含「素食需求」 | 改為「蔬食備註」，並移除「素食需求」 |
| S4 表單網址 | 葷食表單 | 蔬食表單（回覆自然分開統計） |

⚠️ 蔬食版多兩列，資訊列群與主卡都會變高：
...
（兩組不是同一個量測寬度，不能直接相減看差值；要比就各自重量一次。）

所以 §S3 那條高度推導鏈算的是葷食版。要照著推蔬食版，記得桌次引導的值變短會少一行、
而餐點安排這一列本身是兩行。

餐點安排原本是主卡外的一塊置中說明（標題＋兩行內文），後來併進資訊列成為一般的一列 ——
因為它跟桌次引導一樣是「這場宴席為你做了什麼安排」，分開兩塊反而讓賓客要看兩個地方。

⛔ **餐點安排排在桌次引導之前，不要按「先座位後餐點」的直覺順序調回去。**
收到蔬食連結的賓客最想確認的是「有沒有我能吃的」，確認之後才會關心坐哪裡。

文案定為「女方親友席宴請蔬食料理，由飯店主廚特別設計，期待與您共享蔬食盛宴」——
緣由是女方媽媽茹素，希望親友宴席吃蔬食。文案沒有點名媽媽（那要她本人同意），
但「期待與您共享」這個收尾讓蔬食讀起來是一份心意，而不是一項限制。
...
> ✅ **最終方案：進站開場層「輕觸開啟」。** 賓客點一下才進入喜帖，那一下同時是音樂的播放許可。
>
> **為什麼繞了一大圈才走到這裡** —— 以下每一條都是實機（iPhone Safari）驗證後才確定的：
>
> | 嘗試 | 結果 |
> | --- | --- |
> | 監聽滑動事件後播放 | ❌ 自動播放政策只認 click／tap，`touchend` 給的 user activation 不算 |
> | `<audio>` 靜音自動播放 | ❌ 被拒 `NotAllowedError`，靜音也不行 |
> | `<video muted autoplay playsinline>` 靜音自動播放 | ✅ 可以（規則對影片較寬鬆）|
> | 滑動時解除靜音 | ❌ **iOS 會在解除靜音的瞬間把媒體暫停** |
>
> **結論：iOS 上沒有任何方式能在「沒有點擊」的前提下發出聲音。**
> 既然一定要有一次點擊，就把它做成進場的儀式，而不是一顆孤零零的播放鈕。
>
> **開場層的規格**：全螢幕覆蓋，底圖用封面同一張（點開後銜接不會有換圖的斷裂），
> 黑色漸層遮罩 `#00000073 → #000000b3`（⛔ 不要用酒紅：疊在照片上會把膚色與白紗染成紫紅），
> 內容由上而下為：英文名／中文名／飾線／「輕觸開啟」，提示文字帶 2.6 秒的呼吸脈動。
>
> ⛔ **開場層不放宴客地點。** 這一層只做一件事：請賓客輕觸。
> 多放資訊會稀釋那個動作，而且賓客點開後馬上就在 S1 看到同樣的內容。
> **地點放在 S1 封面的婚期下方**（見 §9.2.1）。
> **整層本身就是按鈕**，點畫面任何地方都能進入。
>
> ⚠️ **Hero 的入場動畫要等開場層點開才播** —— 蓋著的時候就播完的話，
> 那 2.4 秒的入場序列賓客一眼都看不到。
>
> ⚠️ **開場層期間要鎖住整頁捲動**（`<html>` 與 `<body>` 都要設 `overflow: hidden`，
> 只鎖 body 擋不住）。
>
> ⚠️ **解除靜音時把 `currentTime` 歸零** —— 靜音預播在開場層顯示期間可能已跑了幾十秒，
> 不歸零賓客會從曲子中間聽起。開場層的輕觸是完整授權，這裡的 seek 不會導致媒體被暫停。
>
> ⛔ **媒體已經在播的時候不要再呼叫 `play()`** —— 那會重新觸發自動播放檢查而被拒，
> 若錯誤處理又把它轉回靜音，等於自己把音樂關掉（本案實際發生過）。
...
`aTmJj` 28→16（並改名 `spacer-16`）、`GVVeh` 28→16（並改名 `spacer-16`）、
`E04lOQ` 40→**141**（並改名 `spacer-141`）—— 本輪先落 125，同輪追加調整為 141
（＝40 ＋ 85 上移補償 ＋ 16 視覺微調，使用者看過模擬後決定倒數計時再往下讓 16px）。

⚠️ **桌機版 `HlNtS`（電腦版-段1 Hero）尚未補宴客地點**，該區仍是「婚期 → spacer-48 → 倒數群組」。
本輪依裁決限定手機版，桌機版留待另一個任務。
...
└ 內容欄 342×fit  layout=vertical alignItems=center  pad [59,24,0,24]  gap 0
  ...
  └ 倒數群組 308×80  layout=horizontal gap 12
    └ 格×4 68×80  layout=vertical alignItems=center justifyContent=center gap 4
      ├ 數字 h=34
      └ 標籤 h=12
高度驗算：59+16+20+72+8+26+16+12+16+22+12+20+141+80 = 520（內容底 y=520；實測文字高略小，餘量落在底部留白）

⛔ **上面四個間距（pad-top 59、飾線前 16、婚期前 16、倒數前 141）是一組，不要單獨調其中一個。**
底圖裡兩人的臉落在畫面 y 305–347。初版的排版（pad-top 120、兩個 spacer 28、倒數前 40）
正好讓婚期與宴客地點壓在臉上。改法是把「眉標 → 宴客地點」整組上移 85，
讓地點底部停在 295（頭頂之上 10），**倒數群組不跟著上移** —— 空出來的 85 全部灌進
倒數前的 spacer，臉的四周因此是一整片留白。之後再把飾線以上的三個元素
往下讓 12（兩個 spacer 28 → 16），避免整組貼著畫面頂端。

倒數前的 spacer 141 = 40（原值）+ 85（吃掉上移，倒數回到原位 y 420）+ 16（再往下讓一點）。
拆成這三段記，是因為只有最後的 16 是純視覺偏好，前 85 動了就會破壞上面那組的推導。
往下的餘裕到 30 左右就到頂：Hero 實高 640，SCROLL 提示在 559–596，再往下會擠到它。
⚠️ 底圖是 `object-fit: cover`，裁切位置隨視窗尺寸改變，臉也跟著跑。這組數字是照
337×556（手機開了「放大顯示」時的等效視窗）量的；430 寬實測也成立，但換底圖就要重量。
...
⛔ **桌機雙頁：翻頁時底層只有一半可以馬上換頁。**
翻頁一開始，目前頁的索引就跳到目標頁了，但真實的書翻頁時，另外那半要等紙蓋上才變：

| 方向 | 左頁（照片） | 右頁（文字） |
| --- | --- | --- |
| 往後翻 | 被翻過來的紙**背面蓋上** → 底層**留舊頁** | 紙離開後**露出** → 底層換新頁 |
| 往前翻 | 紙離開後**露出** → 底層換新頁 | 被翻過來的紙**背面蓋上** → 底層**留舊頁** |

兩邊都直接換成新頁的話，會看到「還沒翻，另外半邊就先變成下一張」——
往後翻時左邊照片先跳、往前翻時右邊文字先跳。右頁的頁碼要跟著右頁的**內容**走，
不能跟著索引，否則翻到一半會出現「舊內容配新頁碼」。
