...
電腦版-單頁全覽   x=560  y=0   1440×4050  ← 桌機五段，版式與手機不同（見 §9.5.2）
...
│   （⛔ 桌機版是**一行**，不是手機版的兩行）
├ (spacer 48)
├ 相片列 1120×480  layout=horizontal gap 20  justifyContent=center
│ └ 相片卡 ×3  360×480  radius 20  shadow
│    圖片沿用手機版三張的 prompt（§2.0），三張都要 Generate
├ (spacer 32)
├ ⛔ **桌機不畫頁點列**（設計稿上畫了，那是我的疏漏）
│   理由：頁點是「你在第幾張」的指示器，而桌機三張全部並排看得見，沒有「第幾張」這回事。
│   5 顆點對 3 張照片，語意自相矛盾。實作時桌機隱藏，手機保留。
└ 提示 "點擊放大檢視"  h=18
    ⛔ 文字與手機版不同 —— 桌機沒有滑動手勢，不可寫「左右滑動瀏覽」
高度驗算：96+18+12+68+16+1+24+28+48+480+32+8+20+18+96 = 965 → 超出 880
⚠️ 依此驗算，**段 2 高度改為 980**（不是 880）。長頁總高隨之調整，見下方總表。
```
...
`電腦版-單頁全覽` 實測 **4050**（五段相加 900+980+970+620+580，分毫不差）。
