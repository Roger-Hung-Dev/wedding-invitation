...
| Q11 | S9 是否要顯示英文名 Roger／Ann | 本檔 §13.7 | 無對應元素（本稿未畫英文名；最接近的是 `K52AI`／`me6rS` 手機中文姓名、`DTOGp`／`Q5W16` 桌機中文姓名） | 未處理（逐題內容見 §13.7） |
...
> ✅ **第十八輪解決：靜音預播 ＋ 首次手勢解除靜音，賓客滑動即可聽到音樂。**
>
> **三個實測結論（都與普遍認知不同，⛔ 不要憑印象改回去）**：
>
> **① 滑動無法通過「有聲自動播放」那一關。**
> `touchend` 確實授予 user activation，但 Chrome 的 Autoplay Policy 是**另一套更嚴格的判斷**，
> 要求的是 click／tap。兩者不是同一件事。
>
> **② `<audio>` 即使靜音也不准自動播放。**
> 實測拒絕訊息：`NotAllowedError: play() failed because the user didn't interact with the document first`，
> 且當下 `muted === true`。**「靜音自動播放總是允許」這條規則只適用於 `<video>`。**
>
> **③ 所以背景音樂改用隱藏的 `<video>` 承載 mp3。**
> `<video muted autoplay playsinline>` 的靜音自動播放**是允許的**；音軌一進站就靜音跑起來，
> 首次手勢（含滑動的 `touchend`）只需 `muted = false` —— **解除已在播放的媒體的靜音，
> 不再經過 autoplay policy**，所以滑動就能開聲。
>
> 實測時間軸：進站 3 秒未互動 → 靜音播放中（已播 8 秒）；模擬滑動 → 有聲播放，鈕轉為 playing。
>
> ⚠️ **解除靜音時要把 `currentTime` 歸零** —— 靜音可能已跑數十秒，
> 不歸零賓客會從曲子中間聽起，聽不到前奏。
>
> ⛔ **隱藏那個 `<video>` 不可以用 `display: none` 或 `visibility: hidden`** ——
> 部分瀏覽器會把不可見的媒體視為可回收而停止播放。用 1px ＋ `opacity: 0` ＋
> `pointer-events: none`。iOS 另需 `playsinline`，否則會嘗試全螢幕。
>
> ⚠️ **`play()` 被拒時必須退回 `idle` 並轉回靜音**：留在 `playing` 會讓音樂鈕顯示「播放中」
> 卻沒有聲音，賓客只會以為音量壞了。
>
> ⚠️ **iOS Safari 未實測** —— 其自動播放政策比 Chrome 嚴格，靜音 `<video>` 自動播放
> 在 iOS 上通常允許（需 `playsinline`），但仍待實機確認。
...
| **Q11** | 英文名 `Roger`／`Ann` 是否要出現在這一段 | 現行版面只放中文名 | Hero 已有 `Roger & Ann`，這裡重複可能冗贅；若要加，放在中文名下方 14px `$text-muted` |
