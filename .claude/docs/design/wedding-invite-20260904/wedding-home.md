...
⚠️ **姓名與職業 chip 必須以中線互相對齊。** 兩者要包在同一個
`width: fit-content` 的容器裡（容器寬度＝兩者中較寬的那一個），容器內再置中。

⛔ 分開放**不行** —— 那樣它們只會各自靠左（新娘卡是靠右）對齊，
而姓名與 chip 幾乎不可能一樣寬，看起來就是歪的。
容器本身仍隨 `text-align` 靠左／靠右，所以左右鏡像的版式不受影響。
...
```
S10 Story 390×738   fill=$bg-ivory  layout=vertical alignItems=center  pad [64,24]
├ 眉標 "OUR STORY"                    h=16
├ (spacer 10)
├ 主標 "How We Met"                   h=41
├ (spacer 12)
├ 金線 40×1                            $gold-soft
├ (spacer 20)
├ 書頁卡組 342×414                     ← 木色書皮 12 ＋ 內頁 318×390，見 §14.3.1
├ (spacer 20)
...
逐項加總 `16+10+41+12+1+20+414+20+40+20+16 = 610`，＋上下 padding 128 ＝ **段高 738**（無餘白）。
...
內頁內容 350 ＋ padding 40 ＝ 390，＋ 書皮上下 24 ＝ 卡片組高 **414**。
...
| 頁 | 年份標籤 | 段落標題 | 故事文字 |
| --- | --- | --- | --- |
| 1 | `2019 秋` | `初次見面` | `那年秋天，在朋友的一場聚會上第一次見到彼此。那天散場之後才發現，我們是聊到最後才離開的兩個人，連要回家的方向都一樣。` |
| 2 | `2020 春` | `熟悉起來` | `開始習慣生活裡有對方的日常。她下班傳訊息說今天很累，他就把宵夜送到樓下，說剛好順路——其實那天他整整繞了半座城市才到。` |
| 3 | `2020 夏` | `在一起` | `其實那天沒有誰正式開口說什麼。只是某一天散步回家的路上，牽起來的手就沒有再放開；後來想想，那條路我們一走就走了好多年。` |
| 4 | `2021 – 2025` | `一起走過的日子` | `我們一起搬過三次家，一起吵過架也一起道過歉。日子說不上轟轟烈烈，但每一天都比前一天更確定一點，確定要一直這樣走下去。` |
| 5 | `2026 春` | `他問，她說好` | `就在第一次見面的那家店門口，他單膝跪下。她一邊哭一邊點頭，然後說了一句：你怎麼這麼慢。那天整間店的人全都站起來鼓掌。` |
