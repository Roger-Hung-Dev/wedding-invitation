...
└ 內容欄 342×fit  layout=vertical alignItems=center  pad [59,24,0,24]  gap 0
  ...
  ├ (spacer 125)                            ← 空的，讓臉露出來，見下方說明
  └ 倒數群組 308×80  layout=horizontal gap 12
    └ 格×4 68×80  layout=vertical alignItems=center justifyContent=center gap 4
      ├ 數字 h=34
      └ 標籤 h=12
高度驗算：59+16+20+72+8+26+16+12+16+22+12+20+125+80 = 504（內容底 y=504；實測文字高略小，餘量落在底部留白）

⛔ **上面四個間距（pad-top 59、飾線前 16、婚期前 16、倒數前 125）是一組，不要單獨調其中一個。**
底圖裡兩人的臉落在畫面 y 305–347。初版的排版（pad-top 120、兩個 spacer 28、倒數前 40）
正好讓婚期與宴客地點壓在臉上。改法是把「眉標 → 宴客地點」整組上移 85，
讓地點底部停在 295（頭頂之上 10），**倒數群組留在原處** —— 空出來的 85 全部灌進
倒數前的 spacer（40 → 125），臉的四周因此是一整片留白。之後再把飾線以上的三個元素
往下讓 12（兩個 spacer 28 → 16），避免整組貼著畫面頂端。
總高仍是 504：−61（pad）−24（兩個 spacer）+85（倒數前 spacer）= 0，這是改動有沒有算錯的檢核點。
