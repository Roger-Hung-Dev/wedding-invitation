...
同樣情形也出現在 `venueHall`（`2F　東方明珠`）與 `monogram`（`R　&　A`）。
...
| 欄位預告 | 含「素食需求」 | 不含（賓客是從素食連結進來的，再問一次會讓人以為拿錯連結）|
...
音檔在 `src/assets/audio/wedding-bgm.mp3`（2.1MB、約 3 分 39 秒，循環播放）。
